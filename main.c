...
/* USER CODE BEGIN PV */
// Đây sẽ là phần khai báo biến toàn cục để sử dụng cho hệ thống, các biến này sẽ dùng để cập nhật trạng thái của cơ cấu chấp hành và thông số cảm biến
...
int main(void)
{
  ...
  MX_USART2_UART_Init();
  /* USER CODE BEGIN 2 */
  // Khởi tạo các kết nối với ngoại vi ở đây
...
/* USER CODE BEGIN Header_StartGetSensor */
/**
...
void StartGetSensor(void const * argument)
{
    // Lấy giá trị từ cảm biến và lưu vào biến toàn cục được define ở trên
...
void StartControl(void const * argument)
{
  ...
  /* Infinite loop */
	// Xét tín hiệu điều khiển từ nút bấm và IR
	// Ở đây sẽ xét cho chế độ điều khiển hay tự động, nếu chế độ tự động thì sẽ xét các biến lấy từ cảm biến ở đây để điều khiển
...
void StartDisplayTask(void const * argument)
{
  ...
  /* Infinite loop */
	// Sau khi lấy các giá trị từ cảm biến, hay nhận tín hiệu điều khiển sẽ được gửi đến màn hình hiển thị ở đây.
...
void StartServerTask(void const * argument)
{
  ...
  /* Infinite loop */
	// Sau khi được nhận tín hiệu qua ngắt UART, task này sẽ làm nhiệm vụ check UART buffer để đưa ra tín hiệu điều khiển
	// gửi data lên broker
